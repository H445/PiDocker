...
$CONTAINER = $ContainerName; $IMAGE = "${ImageName}:${ImageTag}"
...
$resolved = if ([IO.Path]::IsPathRooted($BackupFile)) { $BackupFile } else { Join-Path $rootDir $BackupFile }
...
$v = docker volume ls --format '{{.Name}}' 2>$null | Where-Object { $_ -eq $VolumeName }
if ($v) { Write-Host 'Removing volume...'; docker volume rm $VolumeName | Out-Null }
Write-Host 'Creating volume...'; docker volume create $VolumeName | Out-Null
Write-Host 'Extracting backup...'
$mdir = (Split-Path $resolved -Parent) -replace '\\','/' -replace '^([A-Za-z]):','/$1'
$mfile = Split-Path $resolved -Leaf
docker run --rm -v "${VolumeName}:/data" -v "${mdir}:/backup" alpine tar xzf "/backup/$mfile" -C /data
if ($LASTEXITCODE -ne 0) { Write-Error 'Extraction failed.' }
Write-Host 'Recreating container...'
docker run -d --name $CONTAINER -v "${VolumeName}:/root" -v '/var/run/docker.sock:/var/run/docker.sock' $IMAGE tail -f /dev/null | Out-Null
